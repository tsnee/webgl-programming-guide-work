...
"""
  ...
  private def run(
    ...
    registerMouseListeners(canvas, gl, indices.length, 0, uClicked, viewProjMatrix, uMvpMatrix)
    val startTs        = js.Date.now()
    tick(gl, indices.length, 0, uMvpMatrix, viewProjMatrix, startTs)(startTs)
    ...
    canvas.addEventListener(
      ...
      (ev: MouseEvent) =>
        ...
        then
          ...
          val picked    =
            check(gl, numIndices, currentAngle, xInCanvas.toInt, yInCanvas.toInt, uClicked, viewProjMatrix, uMvpMatrix)
  ...
  private def check(
    ...
    draw(gl, numIndices, currentAngle, uMvpMatrix, viewProjMatrix) // cube will be red
    ...
    draw(gl, numIndices, currentAngle, uMvpMatrix, viewProjMatrix) // cube will be its normal color
    picked
  ...
  private def tick(
    ...
    draw(gl, numIndices, nextAngle, uMvpMatrix, viewProjMatrix)
