...
import io.github.tsnee.webgl.{CanvasLookup, ProgramCreator, ShaderCreator}
import org.scalajs.dom
import org.scalajs.dom.{WebGLProgram, WebGLRenderingContext, WebGLShader}
import org.scalajs.dom.html.Canvas
import scala.scalajs.js.JSON

trait SimpleWebglProgram extends CanvasLookup with ShaderCreator with ProgramCreator:
  ...
  def main: Unit =
    lookupCanvasElement(canvasId).flatMap(canvas => Option(canvas.getContext("webgl"))) match
      case None => dom.console.log(s"Cannot get webgl context from HTML Canvas element with ID $canvasId.")
